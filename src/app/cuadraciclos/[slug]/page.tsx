...
interface CuadracicloProductPageProps {
  params: Promise<{ slug: string }>;
}

export async function generateMetadata({ params }: CuadracicloProductPageProps): Promise<Metadata> {
  const { slug } = await params;
  const product = await getProduct(slug);
  if (!product || product.category.parentCategory !== 'cuadraciclos') return {};
  ...
  return {
    ...
    alternates: {
      canonical: `https://yamahacr.com/cuadraciclos/${product.slug}`,
    },
  };
}

export default async function CuadracicloProductPage({ params }: CuadracicloProductPageProps) {
  const { slug } = await params;
  const product = await getProduct(slug);

  if (!product || product.category.parentCategory !== 'cuadraciclos') notFound();
  ...
  return (
    <>
      ...
      <BreadcrumbJsonLd
        items={[
          { name: 'Inicio', href: '/' },
          { name: 'Cuadraciclos & Mulas', href: '/cuadraciclos' },
          { name: product.name, href: `/cuadraciclos/${product.slug}` },
        ]}
      />
