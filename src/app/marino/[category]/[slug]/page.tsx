...
interface MarinoProductPageProps {
  params: Promise<{ category: string; slug: string }>;
}

export async function generateMetadata({ params }: MarinoProductPageProps): Promise<Metadata> {
  const { slug } = await params;
  const product = await getProduct(slug);
  if (!product || product.category.parentCategory !== 'marino') return {};
  ...
  return {
    ...
    alternates: {
      canonical: `https://yamahacr.com/marino/${product.category.slug}/${product.slug}`,
    },
  };
}

export default async function MarinoProductPage({ params }: MarinoProductPageProps) {
  const { category, slug } = await params;
  const product = await getProduct(slug);

  if (!product || product.category.parentCategory !== 'marino' || product.category.slug !== category) notFound();
    ...
    <>
      ...
      <BreadcrumbJsonLd
        items={[
          ...
          { name: product.category.name, href: `/marino/${product.category.slug}` },
          { name: product.name, href: `/marino/${product.category.slug}/${product.slug}` },
        ]}
      />
